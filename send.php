...
$email_to = 'info@ksenos.com.cy'; // Можно несколько, через запятую
...
function si_recaptcha()
{
    if (isset($_POST['g-recaptcha-response']) && !empty($_POST['g-recaptcha-response'])) {
        $secret = '6LdtEbgUAAAAAPJR4nLs7K0Ve5bpdQV6CsedbyTl';
...
switch ($type) {
    ...
    case 1:
        ...
        switch ($extra) {
            ...
            case 4 :
                $text .= ' узнать больше о лояльности';
                $code = 7;
                break;

            case 5 :
                $text .= ' задать нам вопрос';
                $code = 7;
                break;
        }
        break;
    ...
    case 2:
        $mail_theme = 'Заявка c блока "подробности покупки и аренды офиса" ';
        $text .= 'Заявка c блока формы';
        $code = 4;
        break;

    case 3:
        $mail_theme = 'Заявка c блока "подробности покупки и аренды офиса в конце страницы" ';
        $text .= 'Заявка c блока формы 2';
        $code = 5;
        break;

    case 4:
        $mail_theme = 'Заявка c модального окна "Заказать консультацию" ';
        $text .= 'Заявка c модального окна';
        $code = 6;
        break;
}
