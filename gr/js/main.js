jQuery(function ($) {
    ...
    $.fn.SIInit = function () {
        ...
        $('.send-form').SIForms({
            'validateFields': {
                'client_name': 'Εισαγάγετε το όνομά σας',
                'client_phone': 'Εισαγάγετε τον αριθμό τηλεφώνου σας',
                'client_mail': 'Καθορίστε το ηλεκτρονικό σας ταχυδρομείο'
            },
            'checkExtra': function (form) {
                if (!$(form).find('.form-agree-check').hasClass('checked')) {
                    SIPageMessages.show('Για να στείλετε ένα έντυπο, πρέπει να συμφωνήσετε με την επεξεργασία των προσωπικών δεδομένων.');
                    return false;
                }
            },
    ...
    if($('html').find('.wrapper-slider-overview')){
		...
		var overviewSliderTop = new Swiper('.overview-slider-top', {
			spaceBetween: 10,
			slidesPerView: 4,
		...
		overviewSlider.params.control = overviewSliderTop;
    	overviewSliderTop.params.control = overviewSlider;
    }
    ...
    function showInfoModal() {
        if(localStorage.getItem('show-modal-gr')!=1) {
            setTimeout(function(){
                $('.info-show').addClass('active');
                $('.info-show .close').on('click', function(){
                    localStorage.setItem('show-modal-gr', 1);
                    $('.info-show').removeClass('active');
                });
            }, 2000);
        }
    }
    ...
    showInfoModal();
     ...
     $('.item-level').append('<div class="floor-num"></div>');
